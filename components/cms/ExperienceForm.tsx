...
import type { ExperienceFormInput } from "@/app/cms/experience/actions";
import ImageUploader from "./ImageUploader";
import { ChevronDown } from "lucide-react";

type BlockKind = ExperienceBlock["kind"];

function emptyBlock(kind: BlockKind): ExperienceBlock {
    switch (kind) {
        case "paragraph":
...
type Props = {
    initial?: Partial<ExperienceFormInput>;
    onSave: (data: ExperienceFormInput) => Promise<{ error?: string } | void>;
};

export default function ExperienceForm({ initial, onSave }: Props) {
    ...
    const [blocks, setBlocks] = useState<ExperienceBlock[]>(initial?.body ?? []);

    function addBlock(kind: BlockKind) {
        setBlocks((prev) => [...prev, emptyBlock(kind)]);
    }

    function updateBlock(index: number, next: ExperienceBlock) {
    ...
    function handlePreview() {
        ...
        sessionStorage.setItem("experience-preview", JSON.stringify(payload));
        window.open("/experience/preview", "_blank");
    }
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl space-y-6 text-white">
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-medium">Case study body</h2>
